...
if [[ -z "${OCP_VERSION}" ]]; then
    export OCP_VERSION=$(oc version | grep 'Server Version' | awk -F': ' '{print $2}')
fi
...
export RHEL_VERSION=$(podman run --authfile $HOME/pull-secret.txt --rm -ti ${DTK_AUTO} cat /etc/driver-toolkit-release.json | jq -r '.RHEL_VERSION')
